...
public class GestionInvestisseurBeanTest {
	...
	private ServiceInvestisseur serviceInvestisseur;
	 ...
	 * PowerMockito.mockStatic(Utile.class); hs =
	 * Mockito.mock(HttpSession.class); fc = Mockito.mock(FacesContext.class);
	 * connexionBean = Mockito.spy(new ConnexionBean()); serviceConnexion =
	 * Mockito.mock(ServiceConnexion.class);//* // serviceSauvegarde =
	 ...
	 * 
	 * 
	 * //********** // gestionInvestisseurBean = Mockito.spy(new
	 * GestionInvestisseurBean()); // personne = (Personne)
	 * hs.getAttribute("personne");
	 * 
	 * //***********
	 * 
	 * Personne p1 = new Personne(); TypePersonne tp1 = new TypePersonne();
	 * tp1.setIdTypePersonne(3); p1.setTypePersonne(tp1);
	 * 
	 ...
	 * 
	 * personne = new Personne(); personneEntreprise = new Personne();
	 * entreprise = new Entreprise(); gestionInvestisseurBean = new
	 * GestionInvestisseurBean();
	 * 
	 * }
	 * 
	 * /*@Before public void init() { HttpSession hs = Utile.getSession();
	 * personne = (Personne) hs.getAttribute("personne");
	 * setEntrepriseNom("Accenture"); setTypeNom(ACTION);
	...
	public void setUp() {
		...
		serviceInvestisseur = Mockito.mock(ServiceInvestisseur.class);
		...
		Mockito.when(serviceInvestisseur.recupererTitre(
				(Boolean) Mockito.any(), (Boolean) Mockito.any(),
				(Boolean) Mockito.any(), Mockito.anyString(),
				Mockito.anyString(), Mockito.anyString())).then(titres);
		personne = new Personne();
	...
	public void testGetOffresEmisesList() {
		...
		assertEquals(gestionInvestisseurBean.getOffresEmisesList().get(0), o);
	}
	...
	public void testGetOffresList() {
		...
		p.setOffresRecues(offres);
		g.setPersonne(p);

		assertSame(o, g.getOffresList().get(0));
	}
	...
	public void testGetTitresList() {
		...
		assertSame(t, g.getTitresList().get(0));
	}
	...
	public void testGetOffreHistoriquesEmisesList() {
		...
		o.setDateOffreHistorique(new Date(System.currentTimeMillis()));
		offreHistoriques.add(o);

		p.setOffreHistoriquesEmises(offreHistoriques);
		g.setPersonne(p);

		assertSame(g.getOffreHistoriquesEmisesList().get(0), o);
	}
	...
	public void testGetOffreHistoriquesRecuesList() {
		...
		p.setOffreHistoriquesRecues(offreHistoriques);
		g.setPersonne(p);

		assertSame(g.getOffreHistoriquesRecuesList().get(0), o);
	}
